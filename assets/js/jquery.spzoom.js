...
(function ($, undefined) {
  ...
  function Spzoom($element, options) {
    ...
    for (var optionName in defaultOptions) {
      var dataName =
        "spzoom" + optionName.charAt(0).toUpperCase() + optionName.slice(1);
  ...
  Spzoom.prototype.init = function () {
    ...
    if (this.options.showTitle) {
      ...
      if (title) {
        this.$title
          .text(title)
          .css(
            this.options.titlePosition == "top" ? { top: 0 } : { bottom: 0 }
          );
    ...
    var x =
      this.$thumbImage.offset().left +
      (this.$thumbImage.width() - this.$loader.outerWidth()) / 2;
    var y =
      this.$thumbImage.offset().top +
      (this.$thumbImage.height() - this.$loader.outerHeight()) / 2;
  ...
  Spzoom.prototype.render = function () {
    ...
    if (
      this.x < thumb_x ||
      this.x > thumb_x + thumb_w ||
      this.y < thumb_y ||
      this.y > thumb_y + thumb_h
    ) {
      return;
    }
    ...
    var tracker_x = Math.min(
      Math.max(this.x - tracker_w / 2, thumb_x),
      thumb_x + thumb_w - tracker_w
    );
    var tracker_y = Math.min(
      Math.max(this.y - tracker_h / 2, thumb_y),
      thumb_y + thumb_h - tracker_h
    );
